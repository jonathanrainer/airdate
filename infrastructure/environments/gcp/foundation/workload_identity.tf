...
resource "google_iam_workload_identity_pool_provider" "terraform_cloud" {
  ...
  attribute_condition = "assertion.terraform_organization_name.startsWith(\"airdate\")"
}

resource "google_service_account_iam_binding" "terraform_workload_identity_user" {
  members = [
    "principalSet://iam.googleapis.com/projects/${google_project.airdate_foundation.number}/locations/global/workloadIdentityPools/${google_iam_workload_identity_pool.terraform_cloud.workload_identity_pool_id}/attribute.organisation/airdate"
  ]
